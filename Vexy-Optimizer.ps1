...
# Fast native memory status (avoids the WMI/CIM latency of Get-CimInstance Win32_OperatingSystem)
if (-not ([System.Management.Automation.PSTypeName]'VexyNative.MemInfo').Type) {
    Add-Type -Namespace VexyNative -Name MemInfo -Language CSharp -MemberDefinition @'
...
$hudTimer.Add_Tick({
    ...
    try {
        $memStatus = New-Object VexyNative.MEMORYSTATUSEX
        if ([VexyNative.MemInfo]::GlobalMemoryStatusEx($memStatus)) {
