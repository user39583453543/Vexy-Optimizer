            ...
            <Grid Grid.Column="1">
                ...
                <Grid Margin="20">
                                ...
                                <StackPanel>
                                    ...
                                    <Grid>
                                        ...
                                        <Grid x:Name="RingGrid" Width="110" Height="110" HorizontalAlignment="Left">
                                            <Grid.Effect>
                                                <DropShadowEffect x:Name="RingGlow" Color="#3E9CFF" BlurRadius="20" ShadowDepth="0" Opacity="0.7"/>
                                            </Grid.Effect>
                    ...
                    <StackPanel Grid.Column="1">
                        ...
                        <Button x:Name="QaOptimizeNow" Style="{StaticResource ActionCard}" BorderBrush="#3E8CFF">
                            <Button.Effect>
                                <DropShadowEffect x:Name="QaGlow" Color="#3E8CFF" BlurRadius="18" ShadowDepth="0" Opacity="0.6"/>
                            </Button.Effect>
                            <StackPanel>
                ...
                <Grid Margin="30" MaxWidth="640" HorizontalAlignment="Center">
                    <StackPanel>
                        <TextBlock x:Name="TweaksTitleText" Text="OPTIMIZATION" Foreground="#5C8AD9" FontSize="20" FontWeight="Bold" Margin="0,0,0,4"/>
                ...
                <Grid Margin="30" MaxWidth="720" HorizontalAlignment="Center">
                    <StackPanel>
                        <TextBlock Text="STARTUP APPS" Foreground="#5C8AD9" FontSize="20" FontWeight="Bold" Margin="0,0,0,4"/>
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Grid>
        ...
        <StackPanel HorizontalAlignment="Center" VerticalAlignment="Center" Width="420">
            ...
            <TextBlock Text="ENTER ACTIVATION KEY" Foreground="#CFE0FF" FontFamily="Segoe UI" FontSize="14"
                       FontWeight="Bold" HorizontalAlignment="Center" Margin="0,0,0,16"/>
            <Border Background="#0E1730" BorderBrush="#3E8CFF" BorderThickness="1.5" CornerRadius="8">
            ...
            <Button x:Name="KeyActivateBtn" Content="ACTIVATE" Width="200" Height="42" Margin="0,18,0,0"
                    Background="#1657D6" Foreground="White" FontWeight="Bold" BorderThickness="0" Cursor="Hand"/>
...
function Refresh-StartupList {
    ...
    foreach ($entry in $entries) {
        ...
        $row.BorderBrush = [System.Windows.Media.BrushConverter]::new().ConvertFrom("#12203D")
...
function Add-DriftingParticles {
    param($Canvas, $Count, $AreaW, $AreaH)
    $rand = New-Object System.Random
    $neonColors = @("#3E9CFF", "#8C5CFF", "#3EFFE0", "#FF3EDB", "#5C8AD9")
    for ($i = 0; $i -lt $Count; $i++) {
        $size = $rand.Next(2, 5)
        $dot = New-Object System.Windows.Shapes.Ellipse
        $dot.Width = $size
...
$particleTimer.Interval = [TimeSpan]::FromMilliseconds(40)
...
$script:neonPalette = @(
    [System.Windows.Media.Color]::FromRgb(0x3E,0x9C,0xFF),
    [System.Windows.Media.Color]::FromRgb(0x8C,0x5C,0xFF),
    [System.Windows.Media.Color]::FromRgb(0x3E,0xFF,0xE0),
    [System.Windows.Media.Color]::FromRgb(0xFF,0x3E,0xDB),
    [System.Windows.Media.Color]::FromRgb(0x3E,0x9C,0xFF)
)
$script:glowTick2 = 0
...
$window.Add_Loaded({
    ...
    if ($globalParticleCanvas) {
        ...
        Add-DriftingParticles -Canvas $globalParticleCanvas -Count 90 -AreaW $ww -AreaH $hh
        $particleTimer.Start()
    }
    $glowTimer.Start()
    Start-ScanlineSweep
...
function Show-LoadingScreen {
    ...
    # Extra drifting neon particles layered on top, for real motion
    $script:loadParticleData = New-Object System.Collections.Generic.List[object]
    $rand2 = New-Object System.Random
    $neonColors = @("#3E9CFF", "#8C5CFF", "#3EFFE0", "#FF3EDB")
    for ($i = 0; $i -lt 70; $i++) {
        $size = $rand2.Next(2, 5)
        $dot = New-Object System.Windows.Shapes.Ellipse
        $dot.Width = $size
        $dot.Height = $size
        $dot.Fill = [System.Windows.Media.BrushConverter]::new().ConvertFrom($neonColors[$rand2.Next(0, $neonColors.Count)])
        $dot.Opacity = 0.25 + ($rand2.NextDouble() * 0.4)
        ...
        $script:loadParticleData.Add(@{
            Dot = $dot; X = $x; Y = $y
            VX = (($rand2.NextDouble() - 0.5) * 1.4)
            VY = (($rand2.NextDouble() - 0.5) * 1.4) - 0.3
        })
    }
    $loadParticleTimer = New-Object System.Windows.Threading.DispatcherTimer
    $loadParticleTimer.Interval = [TimeSpan]::FromMilliseconds(35)
