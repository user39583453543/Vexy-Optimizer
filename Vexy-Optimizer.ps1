...
$script:VexyVersion = '6.0.0-prototype'
            ...
            <Border Grid.Column="0" Background="#E5090710" BorderBrush="#47215F" BorderThickness="1" CornerRadius="12" Margin="0,0,12,0">
                <Grid><Grid.RowDefinitions><RowDefinition Height="160"/><RowDefinition Height="*"/><RowDefinition Height="145"/></Grid.RowDefinitions>
                    <StackPanel Grid.Row="0" VerticalAlignment="Center">
                        ...
                        <TextBlock Text="OPTIMIZER" HorizontalAlignment="Center" Foreground="#A06DBE" FontSize="10" CharacterSpacing="320"/>
                    </StackPanel>
...
function Initialize-VexyStars {
    param([int]$Count = 90)
    $w = [Math]::Max(1280,[SystemParameters]::PrimaryScreenWidth)
    $h = [Math]::Max(720,[SystemParameters]::PrimaryScreenHeight)
...
$starTimer.Add_Tick({
    ...
    foreach ($s in $script:Stars) {
        $x = [Canvas]::GetLeft($s.Shape) - $s.Speed
        if ($x -lt -4) { $x = $w + 3; [Canvas]::SetTop($s.Shape,$rand.NextDouble()*$h) }
        [Canvas]::SetLeft($s.Shape,$x)
